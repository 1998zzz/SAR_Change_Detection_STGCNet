...
function im = srad( im,delta_t,q0,rho,option,ITER ) 
    for n = 1:ITER 
        % padding elements avoid edges %边缘填充（好像用的是边缘镜像填充）
        % Matlab中好像有自带的边缘填充函数：https://blog.csdn.net/what_lei/article/details/50225917
        [m, n] = size(im);
        W = 1;
        X = zeros(m+2*W, n+2*W);
        ...
        gradRxI = imfilter(im,[0 0 0;0 -1 1;0 0 0],'symmetric');  %imfilter为滤波函数;symmetric表示图像大小通过镜像反射其边界来扩展
        ...
        q2 = 4*del2(im)./(im+1e-10); % del2为离散拉普拉斯算子
